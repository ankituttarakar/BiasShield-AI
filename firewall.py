...
# Change this to match your actual logo file name in /static
...
<body>
  ...
  <img src="{{ url_for('static', filename='logo.png') }}" alt="Firewall Logo">
</div>
...
def analyze_request(url, user_type, data):
    ...
    # Common biased keywords/phrases
    biased_keywords = [
        "only men", "only male", "males only",
        "only women", "only female", "females only",
        "no women", "no men", "exclude",
        "must be", "not allowed",
        "age limit", "younger than", "older than",
        "religion", "caste", "race", "ethnicity", "skin color",
        "country only", "citizens only"
    ]
    ...
    # Check for explicit biased keywords
    for keyword in biased_keywords:
        if keyword in lower_data:
            ...
            output += "🛡 <strong>Firewall Response:</strong> Sent for human review."
            return output

    # Context-specific checks with extracted values
    if user_type == "single":
        for field, pattern in patterns.items():
            ...
            if match and "diverse" not in lower_data:
                ...
                output += "🛡 <strong>Firewall Response:</strong> Flagged for human review."
                return output

    # If no issues found
    output += '<span class="fair-indicator">✅ No Bias Detected:</span> The request appears fair and inclusive.<br>'
